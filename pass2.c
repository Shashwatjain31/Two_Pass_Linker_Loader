/*
	Program : pass2.c
	This prgram uses INPUT.dat and temporray data structure that is ESTAB generated by pass1. 
	Using both the files it actually loads the program in the memory and shows memory snapshot of loaded program also stores this snapshot in OUTPUT.dat file
*/
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
...
int main()
{
 // Variables declaration which will be used to calculate the address of memory and help in loading 
 char temp[10];
 FILE *fp1,*fp2,*fp3;
 ...
 // opening INPUT.DAT and ESTAB.DAT files in read mode and OUTPUT.DAT file in write mode
 ...
 start=loc;
 st=start;
 //loop until End of File occures
 while(!feof(fp1))
 {
	fscanf(fp1,"%s",input);
	if(strcmp(input,"H")==0)
...
// print output to the console
 ...
 ch=fgetc(fp3);
 // print output which is snapshot of the loaded program in the memory on console
 while(ch!=EOF)
 {
  printf("%c",ch);
  ch=fgetc(fp3);
